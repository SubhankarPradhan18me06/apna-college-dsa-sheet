    ...
    <div className="p-8 max-w-6xl mx-auto animate-fade-in">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl font-extrabold text-primary mb-3">{chapter.title}</h1>
          <p className="text-secondary max-w-2xl">{chapter.description}</p>
        </div>
        
        <div className="flex items-center gap-4 bg-bg-card p-4 rounded-2xl border border-surface-border shadow-sm">
          <ProgressRing percentage={progressPercentage} size={60} strokeWidth={5} />
          <div>
            <p className="text-xs text-secondary uppercase tracking-widest font-bold">Progress</p>
            <p className="text-lg font-bold text-primary">
              {chapter.completedProblems} / {chapter.totalProblems} <span className="text-sm font-normal text-secondary">Solved</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-[48px_40px_1fr_100px_120px] gap-4 px-4 py-3 bg-bg-alt/50 border-b border-surface-border text-[10px] font-bold text-secondary uppercase tracking-widest">
